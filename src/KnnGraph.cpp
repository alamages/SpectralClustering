...
void KnnGraph::HandleKneighbors(std::vector<IndexValue>& k_neighbors,
                                IndexValue& neighbor) {
    // if vector is full
    if (k_neighbors.size() >= knn) {
        ...
        if (neighbor.distance < k_neighbors.front().distance) {
            ...
            std::pop_heap(k_neighbors.begin(),
                          k_neighbors.end(),
                          LessIndexValue());
            ...
            std::push_heap(k_neighbors.begin(),
                           k_neighbors.end(),
                           LessIndexValue());
        }
    } else {
        ...
        if (k_neighbors.size() == knn) {
            ...
            std::make_heap(k_neighbors.begin(),
                           k_neighbors.end(),
                           LessIndexValue());
        }
    }
